...
import { DiscountType, InvoiceItem, InvoiceType, Item, ShortOrLong } from "../../../../shared/models/invoice";
...
@Component({
  selector: 'app-invoice-notification-add',
  templateUrl: './invoice-notification-add.component.html',
  styleUrls: ['./invoice-notification-add.component.scss']
})

export class InvoiceNotificationAddComponent implements OnInit {
  ...
  onSubmit() {
    if (this.isFormValid()) {
      if(this.invoiceNotificationForm.valid){
        ...
        this.invoiceService.addNotificationNew(data).subscribe({
          next: data => {
        ...
        this.toastr.error(this.translate.instant('Remarks Required'));
      }
  ...
  addNewProduct() {
    ...
    if (this.productForm.productDescription && this.productForm.quantity > 0 &&
      ...
      console.log(this.productForm.tax.taxName);
      return true;
    }
